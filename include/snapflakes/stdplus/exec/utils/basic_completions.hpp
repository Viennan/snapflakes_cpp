...
#include <snapflakes/stdplus/utilities/guard.hpp>
...
public:
    ...
    constexpr basic_any_completions(basic_any_completions&& other):
...
template<typename Recr, typename Fn>
struct basic_completion_guard_receiver {
    using receiver_concept = stdexec::receiver_t;

    constexpr basic_completion_guard_receiver() = default;

    constexpr basic_completion_guard_receiver(Recr rec, Fn&& f) : rec_(std::move(rec)), f_(std::move(f)) {}

    constexpr ~basic_completion_guard_receiver() = default;

    constexpr auto get_env() const noexcept {
        return stdexec::get_env(rec_);
    }

    template<typename ...Args>
    constexpr void set_value(Args&& ...args) & noexcept {
        auto guard = stdplus::utils::guard([f = std::move(this->f_)]() {
            f();
        });
        stdexec::set_value(rec_, std::forward<Args>(args)...);
    }
    ...
    constexpr void set_value(Args&& ...args) && noexcept {
        ...
        stdexec::set_value(std::move(rec_), std::forward<Args>(args)...);
    }
    ...
    constexpr void set_error(Args&& ...args) & noexcept {
        ...
        stdexec::set_error(rec_, std::forward<Args>(args)...);
    }
    ...
    constexpr void set_error(Args&& ...args) && noexcept {
        ...
        stdexec::set_error(std::move(rec_), std::forward<Args>(args)...);
    }
